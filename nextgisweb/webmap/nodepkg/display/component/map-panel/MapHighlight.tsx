...
export const MapHighlight = observer(function MapHighlight({
    ...
    const colorSF = highlightStore.colorSF
    ...
    useEffect(() => {
        const hlStroke = new Stroke({ width: 3, color: colorSF?.stroke_primary });
        const fill = new Fill({
            color: colorSF?.fill,
        });
        const hlStyle = new Style({
            stroke: hlStroke,
            fill: fill,
            image: new Circle({ stroke: hlStroke, radius: settings.identify_radius }),
        });

        const stroke = new Stroke({ width: 2, color: colorSF?.stroke_primary });
