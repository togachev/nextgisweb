    ...
    for resource, resource_wmg, resource_social in query:
        ...
        if resource_wmg.id != 0:
            if resource.has_permission(PERM_READ, request.user):
                result.append(dict(id=resource.id, owner=True, display_name=resource.display_name, webmap_group_name=resource_wmg.webmap_group_name, action_map=resource_wmg.action_map, preview_fileobj_id=resource_social.preview_fileobj_id, preview_description=resource_social.preview_description))
            if not resource.has_permission(PERM_READ, request.user) and action_map == True:
                result.append(dict(id=resource.id, owner=False, display_name=resource.display_name, webmap_group_name=resource_wmg.webmap_group_name,  action_map=resource_wmg.action_map, preview_fileobj_id=resource_social.preview_fileobj_id, preview_description=resource_social.preview_description))
