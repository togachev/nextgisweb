...
def diagnostics(request) -> JSONType:
    ...
    if layer is not None:
        if lid := layer.get("id"):
            try:
                ...
                raise ValidationError(message=_("NoGEOM layer {} not found!").format(lid))
    ...
    if cid := connection.get("id"):
        try:
            ...
            raise ValidationError(message=_("NoGEOM connection {} not found!").format(cid))
...
def setup_pyramid(comp, config):
    nogeom_connection_factory = ResourceFactory(context=NogeomConnection)
    
    config.add_route(
        "nogeom.connection.nogeom_inspect",
        "/api/resource/{id}/nogeom_inspect/",
        factory=nogeom_connection_factory,
        get=nogeom_inspect_connection,
    )

    config.add_route(
        "nogeom.connection.nogeom_inspect.table",
        "/api/resource/{id}/nogeom_inspect/{table_name:str}/",
        factory=nogeom_connection_factory,
        get=nogeom_inspect_table,
    )

    config.add_route(
        "nogeom.diagnostics",
        "/api/component/nogeom/check",
        post=diagnostics,
    )
